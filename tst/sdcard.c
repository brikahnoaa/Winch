...
void main(void){
  ...
  sysInit();
  mpcInit();
  iodvr=CFGetDriver(void);
  //
  cprintf("\n sdcard read direct access \n");
  //
  if (CFCardDetect()) {
    CFEnable(true);
    ok=ATACapacity(ATADvr iodvr, 
        ulong *sectors, ushort *spt, ushort *heads, void **info);
    cprintf("card has %ld sectors   (status %d)\n", *sectors, ok);
  }
  exit(0);
  //
  ok=ATAReadSectors(ATADvr iodvr, ulong sector, void *buffer, short count);
  ok=ATAWriteSectors(ATADvr iodvr, ulong sector, void *buffer, short count);
}
