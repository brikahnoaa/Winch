...
//*********Upload.c v.1.7b 03/05/2010 Haru Matsumoto**********************
//Installed on Q001 and Q003 for Kona experiment.  
...
//3/01/10  
//
//Bug on the file_blknum inconsitency fixed.//2/28/10
//
...
//layer. If Tx is successfull, "done" signal is sent at the end of TX.  If not 
...
int UpLoad(const struct SerialPort *modem, unsigned int MAX_RECONNECTS)
{
   ...
   /* loop to upload profiles and logs to the remote host */
   else {
		...
		//All good transmissions come back here.  Check the file system first		
		//This loop process eliminates the files one by one after a good TX.
		Beg00: 
		WatchDog();
		max_fid   		=0;
		...
		//Determine the number of files in the file system and the number of blocks needed to transmit these files
		//Find out the min size file.  Files are sent smallest one first, largest last.
				...
				if (strcmp(fname,config_path) != 0) {
					if (strcmp(fname, log_path) != 0) {
						...
						if ((file_len != 0L)) {
							...
							if(file_len>max_file_size){//find out max file fid
		...
		//File splitting, if it is too large to transmit by one TX
		...
		if(num_files !=0){ //still more than one file to send
			...
			do 
			{
			Beg://previous TX bad. Try again with the same file.
				block_num = 1;		 
				last_file = 1;
				/* loop through each fid of the file system */
				//LogAdd("min_fid=%d max_fid %d\n",min_fid, max_fid);
				//for (fid=min_fid; fid<= max_fid; fid++) //HM Now we know the max fid
				//{//
					/* pet the watch dog */
					WatchDog(); 
					...
					if (fioName(fid,fname,FILENAME_MAX)>0)
					{
						...
						{							
							...
							Wait(30000);
							WatchDog();
							//Rev starte here HM added to turn off modem
							//ModemEnable(19200);
				...
				/* check the termination conditions */						
					...
					if (upload_retry <= MAX_UPLOAD_RETRIES) {
						last_blk  = blk_end;					
						Wait(100);
						//Try log again without disconnecting
						goto Beg;
					} else if (file_len <= MaxTXSize) {//if file size too big, don't send & erase
						//Reached the upload limit. Disconnect and try again.
						connect_retry++;
						LogAdd("%d-th hangup out of %d. Reconnect\n",connect_retry,MAX_RECONNECTS);//debug HM
						pputs(modem, "+++", 20, "\n"); Wait(500);
						pputs(modem, "ATH0", 10, "\n");Wait(100);//Hung up statement added by HM
						if(connect_retry>MAX_RECONNECTS){
							hangup(modem);
						...
						WatchDog();
						goto Beg;
					}//file length is too long. Sorry but have to erase it.
					else if(connect_retry>MAX_RECONNECTS) {
						RemoveFile(fid, fid, LastTXstatus);
						goto Err;
						}
				}else if (flag == 2) { //Resend request
					...
					}else if (file_len > MaxTXSize) {//debug HM files are too big, don't send & erase
						LastTXstatus=0;
						LogAdd("File too big. Hung up. %d/%d\n",upload_retry,MAX_UPLOAD_RETRIES);//debug HM
						RemoveFile(fid, fid, LastTXstatus);
						pputs(modem, "+++", 20, "\n"); Wait(500);
						pputs(modem, "ATH0", 10, "\n");Wait(100);//Hung up statement added by HM
						//hangup(modem);
				...
				} else if (flag ==3) { // prepare to download file from land
					// buffer to hold all recieved data
					...
					RemoveFile(fid, fid, 1); //remove one file				
					LogEntry(FuncName, msg);
					...
					do {
						if (modem->getb(&byte)>0) {
							...
							} else if (leader == 6) { //msg legnth lsb
								rbuf[leader-5] = byte;
								leader++;
					...
					if ((((unsigned char) ((ricrc >> 8) & 0x00FF)) == rcrc[0]) && (((unsigned char) (ricrc & 0x00FF)) == rcrc[1])) {
						static cc msg[] = "CRC chk validated. Rcvd blk %i/%i, length %i\n";
						
						//when transmitted, you really have to hang up HM
						pputs(modem, "done", 20, "ATH0\n");Wait(100);
						
						LogEntry(FuncName, msg, rbuf[3], rbuf[4], rilength);
			...
			if(splitTX==1 && (flag==1 || flag==3)){//remove only the file transmitted
				...
				goto Beg00;  //Good blk TX. re-evaluate the file system
			}
		}
		LogAdd("Hung up. %d/%d\n",upload_retry, MAX_UPLOAD_RETRIES);//debug HM
		pputs(modem, "+++", 20, "\n"); Wait(500);
		...
		if ((source=fopen(localpath,"r"))!=0){
			...
			for(j=1; j <= file_blknum; j++){
				...
				if(block_num <=last_blk){ //current blk num is less than the last blk num of resend request
					...
					buf_wait=(length/240)*1000-2000; //HM Calculate wait period between blocks
...
char fname[FILENAME_MAX+1];
	...
	if(fioName(fMaxfid, fname, FILENAME_MAX)>0){
		static cc format[] = "Upload prob. Remove file %s\n"; 
		static cc format2[]= "Upload OK. Remove file %s\n";
