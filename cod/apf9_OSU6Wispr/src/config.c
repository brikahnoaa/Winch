...
extern persistent int ReactivatedByE ;//HM 11/15/2010
...
persistent int ReactivatedByE = 0; //HM 11/17/2010
...
int LogConfiguration(const struct MissionParameters *config,const char *id)
{
   ...
   else
   {
	  ...
	  if(config->user[0]=='E' || config->user[0]=='e') ChGain[0]=config->user[1];
