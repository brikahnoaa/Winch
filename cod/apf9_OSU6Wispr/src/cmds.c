...
static void ManufacturerDiagnosticsAgent(void)
{
   ...
   long val;
   unsigned int crc;
   char ChGain[2], ChMax_Detect_Prof[6], ChDspPwrDpt[5];//HM for DSP control
   ...
   switch (tolower(key))
   {
      ...
      case 'u':
      {
         ...
         if (*buf)
         {
			...
			//DetectTH=10;
			if(Gain >=0 && Gain <10 && Max_Detect_Prof >=0 && DspPwrDpt <2000 && DspPwrDpt >=0){
				csnprintf(buf,sizeof(buf)-1,"Gain=%d, Max # of Detect/Prof=%ld, DSP Pwr ON/OFF Depth=%d",Gain, Max_Detect_Prof, DspPwrDpt);
				pputs(&conio,buf,2,trm);
			} else cputs("Illegal character - do it again", ctrm);	
			//csnprintf(buf, sizeof(buf)-1,"Depth = %c%c%c%c",mission.user[10],mission.user[11],mission.user[12],mission.user[13]);
			//pputs(&conio, buf, 2, trm);
		 }//change end HM
         else cputs(RangeError,ctrm);
...
static void ShowParameters(void)
{
   ...
   csnprintf(buf,sizeof(buf),"APEX version %06lx  sn %04d",FwRev,mission.FloatId); print(buf);
