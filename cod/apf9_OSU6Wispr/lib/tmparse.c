...
int ParseTime(const char *timestr, time_t *sec)
{
   ...
   else
   {
      ...
      else
      {
         ...
         /* process the optional seconds argument */
         if (regs[6].rm_so>0 && regs[6].rm_eo>regs[6].rm_so)
         {
            t.tm_sec  = atoi(extract(timestr,regs[7].rm_so+1,regs[7].rm_eo-regs[7].rm_so));
         }
      ...
      /* release the regex space back to the heap */
      regfree(&regex);
   }
