...
/*------------------------------------------------------------------------*/
/* function to handle trace requests                                      */
/*------------------------------------------------------------------------*/
/**
   This function handles trace requests by printing a diagnostic to
   the logstream.

   \begin{verbatim}
   input:
      function_name ... The name of the function that activated the trace request.
      msg ............. The message to be included with the trace announcement.
   \end{verbatim}
*/
void ftrace(const char *function_name, const char *msg)
{
   /* get the current address at the top of the stack */
   unsigned int Apf9StackAddr; Apf9StackPeek(&Apf9StackAddr);

   doprnt("Trace %s: Stack[base=0x%04x top=0x%04x len=%d size=%d] %s",
          function_name,Apf9StackBase,Apf9StackAddr,Apf9StackLen(),
          Apf9StackSize,msg);
}
