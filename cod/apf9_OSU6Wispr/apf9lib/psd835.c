...
/* object to retain PSD port data during program execution */
unsigned char near Psd835A,Psd835B,Psd835C,Psd835D,Psd835E,Psd835G;

/*------------------------------------------------------------------------*/
/* function to initialize the PSD835 ports                                */
/*------------------------------------------------------------------------*/
/**
   This function initializes the APF9's PSD835 IO ports.  It returns a
   positive value on success and zero otherwise.
*/
int Psd835Init(void)
{
   /* auto power down, turbo off */
   PMMR0 = 0x3a;

   /* connect control signals to PLD AND array */
   PMMR2 = 0x7c;                    

   /* initialize PSD ports */
   Psd835PortAInit();
   Psd835PortBInit();
   Psd835PortCInit();
   Psd835PortDInit();
