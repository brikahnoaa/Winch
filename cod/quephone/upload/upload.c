...
int UpLoadFile(const struct SerialPort *modem, const char *localpath,
	       const long filesize, unsigned char resend_bitmap[], int fid);
int ChkResendReq(const struct SerialPort *modem,
		 unsigned char resend_bitmap[],
		 unsigned char resend_bytes[], unsigned char resend_crc[],
		 int wait);
...
//const int CONF_BUF_SIZE = 256;     //HM
const int REC_BUF_SIZE = 256;	//HM 

//const int REC_BUF_SIZE = 2000;          //HM 
const int MAX_UPLOAD_RETRIES = 3;	//HM
int blk_end = 62;		//largest blocks can be sent
const int BLOCK_SIZE = 4000;	//up to 5000 NOAA, HM
long MaxTXSize = 60000L;	//File larger than this will be split into halves

								  //rename with the last char with A and B.
								  //Recommended value = MaxLogSize/2
int num_commands = 0;		//update no of cmds read  HM 12/12/2014
int max_commands = 5;		//max no of commands you can receive HM 12/12/2014
...
int UpLoad(const struct SerialPort *modem, unsigned int MAX_RECONNECTS)
{
  ...
  // buffer to hold all received commands
  //unsigned char *tbuf = (unsigned char *) malloc(sizeof(unsigned char) * CONF_BUF_SIZE);
  unsigned char *rbuf =
      (unsigned char *) malloc(sizeof(unsigned char) * REC_BUF_SIZE);
  ...
  /* A loop to log on to the remote host and upload files */
    ...
    int numfiles;
    char atrb = 'A';
    int HM_MAXFILES = 10;	//Up to 5 files
    int File_Eval = 0;		//First file system evaluation
  ...
  GoodBeg:
    ...
    WatchDog();
    max_fid = 0;
    splitTX = 0;		//if = 1, stay connected to send a new file
    num_files = 0;		//HM
    max_file_size = 0L;
    ...
    //Find out the smallest file.  In each loop, one smallest file is uploaded. 
    if (File_Eval == 0) {	//first pass only
      HM_MAXFILES = MAXFILES - 1;
      File_Eval = 1;
    } else
      HM_MAXFILES = 10;
    //for (fid=0; fid < MAXFILES-1; fid++) {//MXFILE=156 and takes too long to evaluate
    for (fid = 0; fid < HM_MAXFILES; fid++) {
    ...
    //debug 4 lines 
    //if(max_fid>=0){
    //      static cc format[]="Debug1 Total # of files to send %i flag= %i\n";
    //      LogEntry(FuncName,format,num_files, flag);
    //}

    //Splitting the file too large to transmit by one upload.  Erase the original.
    if (num_files <= 0)
      minFilesize = 0L;		//no files left
	...
	for (jj = numfiles - 2; jj >= 1; jj--) {
	  ...
	  if (fileleng[jj] < minfile)
	    minfile = fileleng[jj];
	  accum += (long) fileleng[jj];
	}
	fileleng[0] = minFilesize - accum;
	if (fileleng[0] >= minfile)
	  fileleng[0] = minfile - 1;
    ...
    //GoodTX:
    //still more file to send or no files but more commands to receive
    //flag=1 done, = 2 resend req, =3 cmds
    if ((num_files > 0 && flag != 1) || (num_files > 0 && flag == 1)
	|| (num_files == 0 && flag != 1)) {
      if (num_files > 0) {
	static cc format0[] = "%ldB %i files %i blks of max %iB Fid %d-%d\n";

	LogEntry(FuncName, format0, total_file_size, num_files, num_blocks,
		 BLOCK_SIZE, min_fid, max_fid);
	fid = minFilesizefid;
	...
	if (fioName(fid, fname, FILENAME_MAX) > 0) {
	  ...
	  if (!strcmp(fname, config_path))
	    continue;

	  /* don't upload the active log file */
	  if (!strcmp(fname, log_path))
	    continue;

	  /* don't upload the active DTX file */
	  //if (!strcmp(fname,DTX_path)) continue;//HM

	  if (CLogin(modem) > 0)	//if not logged in, connect and log on.
	  {
	    /* upload multiple files at a time to the remote host */
	    UpLoadFile(modem, fname, fioLen(fid), resend_bitmap, fid);
		...
		last_blk = blk_end;
		connect_retry++;
		IrModemAttention(modem);	//HM Jan 2015
		goto Rtry;	//try again
	      } else {
	    ...
	    connect_retry++;
	    LogAdd("Connect attempt failed. Try again. Power off/on modem.\n");
	    //pputs(modem, "+++", 20, "\n"); Wait(500);
	    //pputs(modem, "ATH0", 10, "\n"); Wait(100);//Hung up statement added by HM
		...
		goto Err;	// calls limit exceeded
	      ...
	      LogEntry(FuncName, msg);
	      //These 4 lines were added Sept2014 HM
	      ModemDisable();
	      hangup(modem);
	      Wait(10000);
	      goto Err;
	...
	//ResendReq:
	//A file uploaded. Now check if resend request, cmds, or done from land
	//flag=1 done, = 2 resend req, =3 cmds
	resend_blks = 0;
	flag =
	    ChkResendReq(modem, resend_bitmap, resend_bytes, resend_crc, grace);

	if (flag == 1) {	//Good job! Good block TX
	  //debug
	  //static cc format[]="Debug flag1 Total # of files to upload %i\n";
	  ...
	  if (fioName(fid, fname, FILENAME_MAX) > 0) {
	    static cc format[] = "Removing uploaded file %s\n";

	    remove(fname);	//one file only
	  ...
	  //added three lines 
	  if (num_files == 0) {	//last data file sent
	    static cc format[] = "Debug 'done' sent\n";
	...
	/* check the termination conditions */
	  ...
	  static cc message[] =
	      "\nTimeout occurred. Power down/up modem and reconnect.\n";
	  ...
	  num_commands = 0;	//reset commands count here HM 12/12/2014
	  //Wait(3000);                                   
	  WatchDog();
	  ...
	  if (upload_retry <= MAX_UPLOAD_RETRIES) {
	    if ((((unsigned char) ((crctemp >> 8) & 0x00FF)) == resend_crc[0])
		&& (((unsigned char) (crctemp & 0x00FF)) == resend_crc[1])) {
	      //valid CRC     
	      LogEntry(FuncName, message1, resend_blks, upload_retry,
		       MAX_UPLOAD_RETRIES);
	    } else {		//invalid CRC
...
LogAdd("Files are too big. Erase and hung up. %d/%d\n", upload_retry, MAX_UPLOAD_RETRIES);	//debug HM
	  ...
	  // keeps track of how many bytes have been received
	  int leader = 0;
	  ...
	  static cc msg[] = "Reading msg from land.\n";

	  //came here after a successful file TX
	  //total_files_uploaded++;  //increment file numbers uploaded 

	  LogEntry(FuncName, msg);
	  firstTX = 0;		//not 1-st TX any more

	  To = time(NULL);
	  T = To;
	  WatchDog();

	  // Read one line command message from land
	  do {
	    ...
	    /* get the current time *///end of data
	      ...
	      T = time(NULL);
	    /* check the termination conditions */
	    //} while ((T>=0 && To>=0 && difftime(T,To)<10) && (leader < (rilength+5))); //tried 12
	  } while ((T >= 0 && To >= 0 && difftime(T, To) < 20)
		   && (leader < (rilength + 5)));
	  ...
	  if ((((unsigned char) ((ricrc >> 8) & 0x00FF)) == rcrc[0])
	      && (((unsigned char) (ricrc & 0x00FF)) == rcrc[1])) {
	    static cc msg[] =
		"CRC chk valid. Rcvd blk %i/%i, length %i, Files left =%i\n";
	    //pputs(modem, "done", 20, "ATH0\n");Wait(100);
	    LogEntry(FuncName, msg, rbuf[3], rbuf[4], rilength, num_files);
	    //Copy only the good command. Ignore Senddata() command.                                                
	    if (rbuf[5] != 'S')	//if it is not Senddata(), keep the command 
	    {
	      int x = 0;
	      static cc msg[] = "Copying good command. length %i\n";

	      LogEntry(FuncName, msg, rilength);

	      //set the flag to exit recovery
	      LastTXstatus = 1;

	      //Open the file and copy message into it
	      if (Command_exist == 0) {	//if first command. If file exists, it fails. 
	    ...
	    if (num_files > 0) {
	      static cc format[] = "Still %i more files files. 'data' sent.\n";
	    ...
	    //else if(first_file==1){
	      ...
	      static cc format[] = "'done' sent. No more file to send %i\n";
	    ...
	    //goto GoodTX;  //Good blk TX. re-evaluate the entire file system
	...
	//} while ((connect_retry<=MAX_RECONNECTS) && flag != 1); //Repeat to check cmds from the land
      ...
      if (num_files <= 0 && flag == 3)
	num_commands++;		//number of commands after all the files are uploaded 
      if ((num_commands + Command_exist) <= max_commands)
	goto GoodBeg;		//Good blk TX. re-evaluate the file system
      LogAdd("# of commands reached %d. Exiting.\n", max_commands);
      //goto GoodTX;  //Good blk TX. re-evaluate the file system
    }				//No more file && flag=1
    //debug
    LogAdd("Hung up. %d/%d\n", upload_retry, MAX_UPLOAD_RETRIES);	//debug HM
    //debug end
    ...
    Wait(500);			//Hung up 
    //hangup(modem);
    //LogAdd("# of effective commands, %d\n", Command_exist);
    if (Command_exist > 0) {
      static cc msg[] = "Creating mission.cfg\n";

      fclose(config);
      free(rbuf);
      remove(config_path);
      rename("rxconfig.tmp", config_path);	//rename to mission.cfg
      Command_exist = 0;
      //Tell config that we have a new mission.cfg file
      vitals.status &= ~DownLoadCfg;
      LogEntry(FuncName, msg);
  ...
  else if (block_num <= last_blk)	//skip after the last bad blk is sent to save time
  {
	...
	if (block_num <= last_blk) {	//current blk num is less than the last blk num of resend request
	  if (j == file_blknum) {	//If we are transmitting the last block in this file
	    if (j == 1) {	//If the last block is the first block (ie, the file fits in one block)
		  ...
		  (unsigned char *) malloc((sizeof(unsigned char)) *
					   (filesize + 25 + filenamesize));

	      //Read the file into the buffer and get the length
	      length =
		  fread(buf + 22 + filenamesize, sizeof(unsigned char),
			filesize, source) + 17 + filenamesize;
	      //LogAdd("a.Length=%d\n", length);
	      ...
	      //Read the file into the buffer and get the length
	      length =
		  fread(buf + 10, sizeof(unsigned char),
			filesize - BLOCK_SIZE * (j - 1), source) + 5;
	      //LogAdd("b.Length=%d\n", length);
	    ...
	    buf =
		(unsigned char *) malloc((sizeof(unsigned char)) *
					 (BLOCK_SIZE + 25 + filenamesize));

	    //Read the file into the buffer and get the length
	    length =
		fread(buf + 22 + filenamesize, sizeof(unsigned char),
		      BLOCK_SIZE, source) + 17 + filenamesize;
	    //LogAdd("c.Length=%d\n", length);
	    ...
	    //Read the file into the buffer and get the length
	    length =
		fread(buf + 10, sizeof(unsigned char), BLOCK_SIZE, source) + 5;
	    //LogAdd("d.Length=%d\n", length);
	    ...
	    if (block_num == last_blk)
	      fclose(source);	//make sure file is closed
	  }

	  // Create a char with the bit we need to check for this particular block
	  temp2 = (char) ((int)
			  (pow((double) 2, (double) ((block_num - 1) % 8))));
	  ...
	  if ((temp2 & resend_bitmap[(block_num - 1) / 8]) == temp2) {	// Check if the block is set in the bitmap
	    ...
	    LogEntry(FuncName, format, localpath, length, block_num,
		     num_blocks, j);
	    ...
	    if (pputs(modem, "@@@", 3, ""));
	    Wait(200);
	    if (pputb(modem, buf[3], 1));
	    Wait(50);
	    if (pputb(modem, buf[4], 1));
	    Wait(100);
	    for (iterator = 5; iterator < length5; iterator++) {
	      pputb(modem, buf[iterator], 100);
	    }
	  ...
	  buf_wait = (length / 300) * 1000;	//HM Calculate wait period between blocks 12/15/2014
	  if (buf_wait < 0)
	    buf_wait = 0;
	  //if((last_file && (j == file_blknum)) || skip ==1 || num_blocks==block_num || block_num == last_blk) buf_wait=0; //if it is the last buffer, no wait. HM
	  if ((j == file_blknum) || skip == 1 || num_blocks == block_num
	      || block_num == last_blk)
	    buf_wait = 0;	//if it is the last buffer, no wait. HM
		 ...
		 int wait)
{
  ...
  do {				//check response
	...
	if (byte == '\r')
	  ...
	  if (byte == '@')
	    byte = 'a';		//@@@ signs forbidden on the record
	  LogAdd("%c", byte);
	} else
	  LogAdd("[0x%02x]", byte);
      }
	...
	if (expect(modem, "mds", "", 10, "") > 0) {
      ...
      } else if (byte == 'm') {	//Land is sending "cmds" msg, success HM
	LogAdd("c%c", byte);
...
void RemoveFile(int min_fid, int max_fid, int LastTXstatus)
{
  ...
  for (fid = min_fid; fid <= max_fid; fid++) {
	...
	if (strcmp(fname, log_path) != 0) {
	  if (fioLen(fid) > fMax)
	    fMaxfid = fid;	//find the largest file
	}
      }
    }
  }
