...
int UpLoad(const struct SerialPort *modem, unsigned int MAX_RECONNECTS)
{
   ...
   /* A loop to log on to the remote host and upload files */
		...
		if(num_files<=0)minFilesize=0L;//no files left
		...
		if((num_files >0 && flag !=1)||(num_files >0 && flag==1)||(num_files==0 && flag !=1)){ 
			if(num_files > 0){
			...
			Rtry://previous TX was bad. Try again with the same file.
				...
				// Prepare to download msg file from land
					...
					} while ((T>=0 && To>=0 && difftime(T,To)<20) && (leader < (rilength+5))); 
			...
			if(num_files <=0 && flag == 3) num_commands++; //number of commands after all the files are uploaded 
			if((num_commands+Command_exist) <= max_commands)goto GoodBeg;  //Good blk TX. re-evaluate the file system
	...
	if(total_files_uploaded!= 0){
		...
		if (LastTXstatus==0)
		{
		  ...
		  if (Command_exist>0) fclose(config);//HM
		}	   
	}
	//DspOff();//HM debug
	return LastTXstatus;
