...
persistent int ReactivatedByE = 0; //HM 11/15/2010
persistent int RecoveryMode = 0;//HM
...
/**
   ...
   MissionParameters parameters:
      ...
      Ascent_Time_Out ............ The initial segment of the uptime that is
      ...
      Con_Time_Out ........... The number of seconds allowed after dialing
   ...
   A sample configuration file is below.
      ...
      # user name (G1D000020P0400) G1 is gain. D0000020 is the maximum detection limit
					per profile.  If the detection exceeds this value, QUEphone stops
					parking and starts profiling (come up). P0400 is the threshold value can be 
					passed on to the DSP.
					
      # User(iridium) [0x835f]  - Legacy
...
int configure(struct MissionParameters *config, const char *fname)
{
   ...
   else
   {
      ...
      enum {ActivateRecoveryMode, AirBladderMaxP, Ascent_Time_Out, AtDialCmd, AltDialCmd,
	      BuoyancyNudge, CompensatorHyperRetraction, Con_Time_Out, CpActivationP,
      ...
      parms[Ascent_Time_Out]            = "ascenttimeout"; //12/17/2015 HM
      ...
      parms[Con_Time_Out]               = "connecttimeout";  //12/17/2015 HM
      ...
      if (status>0)
      {
         ...
         /* check if recovery mode is enabled */ //HM 11/15/2010
         if (RecoveryMode==1) 
         {
...
static int ConfigSupervisor(struct MissionParameters *cfg)
{
   ...
   /* check for sensible piston positions */
...
//When the float receives a new command User(G2D000020P0400) for example, 
//it changes Gain=2, and Max_Detect_Prof=20, DSP turn on/off depth is 400m.
//HM 11/2/2009
#include <eeprom.h> //HM 11/18/2010
#include <control.h> //HM 11/18/2010
int LogConfiguration(const struct MissionParameters *config,const char *id)
{
   static cc FuncName[] = "LogConfig..()";
   int status=-1;
   char ChGain[2], ChMax_Detect_Prof[8], ChDspPwrDpt[6];
   ...
   else
   {
	  ...
	  //Check if Reactivating command 11/15/2010 HM Start
	  if(config->user[0]=='E' || config->user[0]=='e')
		/* Re-execute the mission */ 
		{
            /* read the mission state from EEPROM */         
            /* check if the mission is presently recovery mode */
            if (StateGet() == -1)//=it was in RECOVERY mode. 
			{
				/* create the message */
				static cc msg[]="Reactivation initiated remotely from recovery mode %d, %d.\n";
				ReactivatedByE=1; //Mission was reactivated by remote HM
				RecoveryMode  =0; //HM
				/* log the message */
				LogEntry(FuncName,msg, ReactivatedByE, RecoveryMode);
				/* put the float into surface diagnostics mode */
				if (MissionLaunch()>0)
					{
					/* notify that the mission has been activated */
					/* create the message */
					static cc msg[]="Mission reactivation successful.\n";
		...
		//end HM
	  if(StateGet()==1){//Prelude phase. Initialize the mode
		//static cc msg[]="RecoveryMode=%d, ReactivatedByE=%d.\n";
		if(ReactivatedByE !=1){
			RecoveryMode=0;
			ReactivatedByE=0;
		}//LogEntry(FuncName, msg, RecoveryMode, ReactivatedByE);
	  }
	  //RecoveryMode=0; //Reset the mode HM WARN!!!
	  //Scan the new Gain and Detection parameters HM
	  if(config->user[0]=='G' || config->user[0]=='g') ChGain[0]=config->user[1];
	  if(config->user[0]=='E' || config->user[0]=='e'){
	    ChGain[0]=config->user[1];
	    mission.user[0]='G';//Change back to the normal mission mode 12/23/2014 HM
	   }
	  ...
	  ChGain[1]=0;			//make the last digit NULL
	  Gain=atoi(ChGain);	//define a new gain
	  ChDspPwrDpt[4]=0;		//make the last digit NULL
	  DspPwrDpt = atoi(ChDspPwrDpt);  //define a new DSP power on/off depth
	  ...
	  if(Gain<4 && Gain >=0 && DspPwrDpt<10000 && DspPwrDpt>=0) {
		QuecomSetNCM(Gain, DspPwrDpt);
	  }
	  //End HM
