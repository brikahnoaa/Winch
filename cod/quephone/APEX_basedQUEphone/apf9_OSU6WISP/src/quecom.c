...
persistent int HMdebug=0;
int Reboot_OK=1;					//HM Dec2015
persistent int	No_Response;					//HM Dec2015
 ...
 * Also added a feature to monitor the response from WISPR.  If no response >=3,
 * it reboots the WISPR.
 */
long QuecomGetDTX(long max_ndtx)
{
   ...
   bufsize = sprintf(buf, "$DX?,%d*", (int) max_ndtx);//Nov2015 HM   
   if (QuecomWrite(buf, bufsize, 0) != bufsize) {  //Open Com1 and leave it open
      static cc msg[] = "message write error.\n";
      LogEntry(FuncName, msg);
   }
   
   /* Wait for DXN message from the DSP should be 59 char message = $DXN,xxxxx* */
   ...
   if ((debuglevel >= 3) || (HMdebug == 1)) { //HM1224
   ...
   if ((strncmp(&buf[1], "DXN", 3) == 0)) {//HM original
		...
		if (ndtx > 0) {	
			...
			if((phase == DESCENT)||(phase == PARK)||(phase == GODEEP)||(phase==PROFILE)){
				//duration is not sent from WISPR.
				//static cc format[] = "Ndtx=%ld,Nclk=%ld,Acm=%ld,MTK=%ld,MTHR=%ld,MRT=%ld,MICI=%0.2f,MDUR=%0.2f,P=%0.1fdB\n";
				static cc format[] = "Ndtx=%ld,Nclk=%ld,Acm=%ld,MTK=%ld,MTHR=%ld,MRT=%ld,MICI=%0.3f,P=%0.1fdB\n";
				//LogEntry(FuncName, format, ndtx, nclk,accum_dtx, m_tk, m_th, m_rt, fm_ici, fm_dur,pres);
				LogEntry(FuncName, format, ndtx, nclk,accum_dtx, m_tk, m_th, m_rt, fm_ici, pres);
			}
		}
		Psd835PortAClear(PTT_TX_ON); Wait(10); //keep the WISPR on HM Dec2015
		
		//If accumulated detection exceeds, it starts ascending
	...
	//else if (debuglevel>=1){ //HM1224
	else{  //HM1224
		No_Response++;
		/* create the message */
		if(Reboot_OK == 1){
			static cc format[]="No response WISPR %d\n";
			/* make the logentry */
			LogEntry(FuncName,format, No_Response);

			if (No_Response>=3){
				static cc format[]="No detection updates %d times. Turn off/on WISPR.\n";
				LogEntry(FuncName, format, No_Response);
				//Turn off DSP and on again
				DspOff();
				DspStart();    //Dec, 2015
				No_Response=0; //reset
			}
...
/***************************************************************************
QuecomSetGain - Send a new gain value.  It should be sent between 20 to 28 sec
