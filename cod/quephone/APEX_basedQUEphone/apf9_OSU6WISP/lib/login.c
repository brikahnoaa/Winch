...
int login(const struct SerialPort *port,const char *project_id, const char *platform_id)
{
   ...
   else
   {
      ...
      else
      {
		 ...
		 proj[0]='Q';proj[1]='U'; proj[2]='E'; proj[3]= 'H';
         ...
         /* give multiple chances to login before aborting */	
         /* check if carrier-detect enabled and CD line not asserted */
         if (port->cd && !port->cd())
         {
            /* create the message */
		 ...
		 //No need this extra CR HM debug
         ...
         while (num_tries < 3 && temp_flag == 0) {
			pputs(port,buf, timeout,"");
			Wait(1000);
			pputs(port,buf, timeout,"");
			//Wait(2000);
			//pputs(port,buf, timeout,"");
            ...
            if ( result <= 0) {
			  ...
			  /* make the logentry */
              LogEntry(FuncName,msg);
            //} else if (expect(port,"ACK", "", 12, "") <= 0) {
            } else if (expect(port,"ACK", "", 30, "") <= 0) {
