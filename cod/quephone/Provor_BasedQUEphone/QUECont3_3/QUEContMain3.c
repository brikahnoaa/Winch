/******************************************************************************\
** QUEContMain3.c				Persistor CF2 program
**
...
**	31	IRID/GPS RXOUT			TPU10
**	32	IRID/GPS TXIN			TPU11
**	33	PROV RXOUT				TPU12
**	34	PROV TXIN				TPU13
**	35	OPEN					TPU14
**	37	OPEN					TPU15
...
//	DAQ board definition
  ...
  //#include	<ADS8345.h>		// ADS8344/8345 SPI A-D QPB Driver for CFX 
...
IEV_C_PROTO(ADSamplingRuptHandler);	// implied (IEVStack *ievstack:__a0) parameter
...
int main()
{
	...
	while(QUE.GOTODIVE == 1) //You have a permission to dive
		...
		if(phase <4)
			...
			//Check if oil flows.  If pumped too long, oil may not flow well.  
			CheckResOpenValve(3000);
		...
		if((LogOnly || SatTerm) && (phase ==0 || phase ==1))//Still at surface
			...
			if(SatComOpen)//Com open and antenna is GPS
		  	...
		  	//**Supposed to receive GoToDive instruction from the land 
			...
			{//The float was already submerged state, and came here because of reset
  ...
  //**Came here because IRIDIUM transmission was successful and ready to dive or already in dive mode
		if((ProvTerm || LogOnly) && QUE.GOTODIVE)
			...
			if((phase == 2) && ProvTerm) //Advanced to Surface_Dive mode
				...
				while(phase <3)
					...
					TimeUp=ListenProvorLowPower(shortwait, 32);
					} 
				WritePhase();
			...
			if((phase == 3) && ProvTerm) //Full dive mode
				...
				while(phase <4)
					{
					TimeUp=ListenProvorLowPower(longwait, 256);
		...
		//Grounded bottom! 
			...
			if(phase == 5 && QUE.GOMONITOR !=1 || LogOnly)//2-way com now.
				{
				BIRS.PREVDEPTH = (short)((float)pressure/pConv);	
				flogf("REACHED BOTTOM AT %dM %ddBars\n", BIRS.PREVDEPTH, pressure);cdrain();coflush();
				result=10;
				Delay(10);
				QUE.GOMONITOR = 1;
				sprintf(strbuf, "%u", QUE.GOMONITOR);
			...
			if((phase > 3  && phase < 8) || LogOnly)
				...
				while(QUE.GOMONITOR ==1)
					...
					if(!SetupAcquisition())
						{
						//debug
						for(filecount=1;filecount<=maxfile;++filecount)
							{
							if(filecount>1)//going to dive for the first time
								{
								sprintf(&fnew[2],"%08lu.DAT",(long)++QUE.FILECOUNT);//name a new detection file
								sprintf(strbuf, "%u", QUE.FILECOUNT);
								VEEStoreStr(QUE_FILECOUNT_NAME,strbuf);
						...
						//debug end
							if(Acquire(fnew)==-1)break;//you only need this line
						//debug
				...
				if(ProvTerm)
					...
					if(phase==4 || phase==5 )//previous status
						{//find out which current status it is now.  This may take a while.
						result=InqProvorStatus(&pStatus, &lvStatus, &phase, &pDur,	&vDur, &pressure);		
						if(result<1)flogf("PROVAR STATUS %d,%d,%05d,%05d,%d,%04d\n",pStatus,lvStatus,pDur,\
						vDur, phase, pressure); cdrain(); coflush();//status is 4, now accepts command
						}
					else
						{//previus status was 6 or 7, wait until phase becomes 4
						RepInq=(QUE.PARKSAMPL *60)/longwait+2;
						while (phase !=4)ListenProvorLowPower(longwait, RepInq);
						}
			...
			if(phase >7 && ProvTerm) 
				...
				while(phase !=1)
					{
					ListenProvorLowPower(longwait, 256);
...
*/	
  ...
  //**IRIDUM calls exeeds maximum	
	...
	while(!ProgramFinished && QUE.NUMDRFTCL < QUE.MAXDRFTCL && (SatTerm || LogOnly))//until interrupted
		{
  //**Turn on GPS/IRIDIUM unit and get GPS time and position
  		flogf("**************REPORTING GPS WHILE DRIFTING TX#=%d***********\n", QUE.NUMDRFTCL);cdrain();coflush();
		if(ProvTerm)
			{
			if(!ProvComOpen)ProvComOpen=OpenCommProv();
...
//	may change and keeps floaters from wasting power.
	...
	if(ProvTerm && (QUE.STATUS==0 || QUE.STATUS==1 || QUE.STATUS==4 || QUE.STATUS==5))
		...
		result=10;
		flogf("COM MAY BE ONE-WAY. STATUS =2,3,6,7,8, OR 9. WAIT %dSEC FOR RESPONSE.\n",longwait);
			RTCDelayMicroSeconds(10000L);
		while(result > 1)
			{
...
//	CFPPBSZ		CompactFlash ping-pong buffer size in bytes.
...
// Keep CF2 low power interrupt mode while listening to Provor using IRQ2.
// It keeps listening until specified "waitSec" is up or repeats RepeatMax times.  
// If nothing happens, it sends an inquirey to PROVOR.  
// "waitSec" is approximately 300 (set 330) sec during 3, 6 and 7. 
...
bool ListenProvorLowPower(short waitSec, short RepeatMax)
	...
	while(phase == QUE.STATUS && Repeat < RepeatMax)
		...
		while(PinRead(DFLIP)){PinClear(DFLIP);RTCDelayMicroSeconds(10);}	
		...
		while(PinRead(IRQ2) && !TimeUp)    // we saw TU serial activity at PROVOR
			...
			if(TimingRuptCount > waitSec)TimeUp = true;//waiting is over 
			}
		TickleSWSR();		// Ping to keep active & safe from watch-dog reset 
	...
	//the point of the current position
{
	...
	j= (Lat-refLat+0.5)/delta;
	i= (Lon-refLon+0.5)/delta;
	//printf("%d %d\n", i,j);
...
//AveLat=45.9990;
//AveLon=-130.0000;
//sprintf(loc_chr,"N45:59.944W130:00.000");
//WriteLocEPROM('N', 45, 59.944, 'W', 130, 0.000);//Write new location to VEEPROM
...
AveLat=44.6243;
AveLon=-124.0354;
//sprintf(loc_chr,"N44:37.283W124:02.643");
//sprintf(LatChar,"N44:37.283");
//sprintf(LonChar,"W124:02.643");
...
**	
\******************************************************************************/
IEV_C_FUNCT(ADSamplingRuptHandler)	// implied (IEVStack *ievstack:__a0) parameter
	{
	#pragma unused(ievstack)
	...
	QPBClearInterrupt();
	ADDataBuf[ADRawHead] = * (RawData *) CFxADQueueToArray(ad, (void *) QRR, NCHAN);
	ADRawHead = (ADRawHead + 1) & AD_RAW_DATA_BUF_MASK;
// Add overflow check here: if (ADRawHead == ADRawTail) ...trouble...
	//DBG( PinSet(27); )
...
short SetupAcquisition(void)
	...
	AD_RAW_DATA_BUF_SIZE=QUE.CFPPBSZ;	// whatever is appropriate
	AD_RAW_DATA_BUF_MASK=((AD_RAW_DATA_BUF_SIZE / (NCHAN * 2)) - 1);
	freeSpace = DIRFreeSpace("C:\\");
	totalSpace= DIRTotalSpace("C:\\");
    //BIRS.BIRStartFreeKB=freeSpace/1000;
    //BIRS.BIRCapacityKB=totalSpace/1000;	
	...
	ADDataBuf = (RawData * ) malloc(AD_RAW_DATA_BUF_SIZE);
	if (ADDataBuf == 0)
		{
		flogf("Couldn't allocate the raw data buffer!\n");
		return 1;
		}
...
short Acquire(char *fname)
	...
	if(QUE.SAMPLES==2 || QUE.SAMPLES==3)dsize=2L;
	else dsize=1L;
	sample=QUE.CFPPBSZ/dsize; //total A/D data number
	IEVInsertCFunct(&ExtFinishPulseRuptHandler, level5InterruptAutovector);
	PinBus(IRQ5);					// enable PBM (IRQ5) interrupt button
	...
	//while (PinRead(IRQ4RXD))	// and go until a key gets hit
		...
		if (QPBTestRunning())			  // conversions in process?
			...
			*(ushort *)0xffffe00c=0xF000; //force CF card into Card-1 active mode
			LPStopCSE(FastStop);		  // VCO on (immed. wake) CPU off, SIMCLK is VCO clock
										  // RTC function does not work properly.
			//LPStopCSE(LPMODE);			  // VCO on (immed. wake) CPU on, SIMCLK is VCO clock
			}
		}
	...
	flogf("END LOGGING   %.20s\n", End_Chr);putflush();
	//flogf("Elapse time %lu us\n", elp);
...
bool CheckResOpenValve(short duration)
	...
	if(ProvTerm && (phase ==0 || phase ==1))
		...
		result=10;
		while(result>1) result = GetReservoirLevel(&reservLevel);
		cprintf("RESERVOR LEVEL = %dcc\n", reservLevel);cdrain();coflush();RTCDelayMicroSeconds(10000L);
		while(reservLevel == 0)
			{
			reservLevel=BalanceReservoir(duration);
			TickleSWSR();
			}
		uprintf("RESERVOIR IS BALANCED\n");cdrain();coflush();	
			}
	return true;
	}
